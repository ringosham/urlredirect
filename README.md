...
Takes about ~11MB of memory pretty much all the time.

## Usage (Host side)

A systemd unit is provided but its simple enough to run it on any init systems

1. Build the project using `go build -o urlredirect urlredirect/server`.
2. Just run it with `./urlredirect` and it will start listening on port 10080.
